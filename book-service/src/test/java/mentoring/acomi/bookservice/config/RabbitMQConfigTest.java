...
public class RabbitMQConfigTest {
	...
	Declarables bookBindings(Queue bookQueue, TopicExchange eventsExchange) {
		return new Declarables(
				...
				BindingBuilder.bind(bookQueue).to(eventsExchange)
						.with(IntegrationEventTypes.BOOK_REQUEST_ADDED.getRoutingKey()),

				BindingBuilder.bind(bookQueue).to(eventsExchange)
						.with(IntegrationEventTypes.BOOK_REQUEST_APPROVED.getRoutingKey()),
	...
	Declarables notificationBindings(Queue notificationQueue, TopicExchange notificationsExchange) {
		return new Declarables(BindingBuilder.bind(notificationQueue).to(notificationsExchange)
	...
	Declarables bookNotificationBindings(Queue bookNotificationQueue, TopicExchange notificationsExchange) {
		return new Declarables(BindingBuilder.bind(bookNotificationQueue).to(notificationsExchange)
	...
	@Bean
	Queue testQueue() {
		return QueueBuilder.durable("test.queue").deadLetterExchange("dlq-test.dlx")
				.deadLetterRoutingKey("test.routing.key").build();
	}
	...
	Binding testQueueBinding(Queue testQueue, TopicExchange eventsExchange) {
		...
		return BindingBuilder.bind(testQueue).to(eventsExchange).with("test.routing.key");
	}

	@Bean
	Binding deadLetterBinding(Queue deadLetterQueue, FanoutExchange dlx) {
		return BindingBuilder.bind(deadLetterQueue).to(dlx);
	}
