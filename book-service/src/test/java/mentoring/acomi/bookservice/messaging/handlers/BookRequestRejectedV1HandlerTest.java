...
public class BookRequestRejectedV1HandlerTest extends AbstractEventHandlerTest {

	@Mock
	private BookRequestProjectionOperations projectionOperations;
	
	private BookRequestRejectedV1Handler handler;
	
	@BeforeEach
	void setUp() {
		handler = new BookRequestRejectedV1Handler(projectionOperations, mapper);
	}
	...
	@Override
	protected IntegrationEventEnvelope<BookRequestRejectedIntegrationPayload> validEvent() {
		return getBookRequestRejectedEvent(UUID.randomUUID().toString(), 1);
	}
	
	void shouldHandleBookRequestRejectedEvent() {
		IntegrationEventEnvelope<BookRequestRejectedIntegrationPayload> event = validEvent();
		Optional<ProjectionUpdateNotification> notification = handler.handleEvent(event);
		Assertions.assertTrue(notification.isPresent());
		verify(projectionOperations, times(1)).reject(event.payload().requestId(), event.occurredAt());
	}
	...
	private List<InvalidPayloadScenario> invalidPayloads() {
	    return List.of(new InvalidPayloadScenario("blank requestId", "requestId", getBookRequestRejectedEvent("", 1)));
	}

	private IntegrationEventEnvelope<BookRequestRejectedIntegrationPayload> getBookRequestRejectedEvent(String requestId, int schemaVersion) {

		String aggregateId = requestId == null || requestId.isBlank() ? UUID.randomUUID().toString() : requestId;
		
		return new IntegrationEventEnvelope<>(UUID.randomUUID().toString(), IntegrationEventTypes.BOOK_REQUEST_REJECTED,
				"test-handler", aggregateId, AggregateType.BOOK_REQUEST.name(), 0, Instant.now(),
				schemaVersion, new BookRequestRejectedIntegrationPayload(requestId, ""));
	}
}
