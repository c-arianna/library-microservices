...
public class BookReleasedV1HandlerTest extends AbstractEventHandlerTest {
	...
	void setUp() {
		...
		handler = new BookReleasedV1Handler(projection, mapper);
	}
	...
	void shouldRejectPayloadWithBlankIsbn() {
		...
		IntegrationEventEnvelope<BookLoanIntegrationPayload> event = getBookReleasedEvent("", UUID.randomUUID().toString(),  
	...
	void shouldRejectPayloadWithBlankLoanId() {
		...
		IntegrationEventEnvelope<BookLoanIntegrationPayload> event = getBookReleasedEvent("9788804336327", "", UUID.randomUUID().toString(), 1);
	...
	protected IntegrationEventEnvelope<?> withSchemaVersion(int schemaVersion) {
		return getBookReleasedEvent("9788804336327", UUID.randomUUID().toString(),  UUID.randomUUID().toString(), schemaVersion);
	}
	
	private IntegrationEventEnvelope<BookLoanIntegrationPayload> getBookReleasedEvent(String isbn, String loanId, String userId, int schemaVersion) {
