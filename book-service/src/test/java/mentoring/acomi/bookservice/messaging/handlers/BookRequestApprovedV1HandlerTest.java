...
public class BookRequestApprovedV1HandlerTest extends AbstractEventHandlerTest {

	@Mock
	private BookRequestProjectionOperations projectionOperations;
	
	private BookRequestApprovedV1Handler handler;
	
	@BeforeEach
	void setUp() {
		handler = new BookRequestApprovedV1Handler(projectionOperations, mapper);
	}
	...
	@Override
	protected IntegrationEventEnvelope<BookRequestApprovedIntegrationPayload> validEvent() {
		return getBookRequestApprovedEvent(UUID.randomUUID().toString(), 1);
	}
	
	void shouldHandleBookRequestApprovedEvent() {
		IntegrationEventEnvelope<BookRequestApprovedIntegrationPayload> event = validEvent();
		Optional<ProjectionUpdateNotification> notification = handler.handleEvent(event);
		Assertions.assertTrue(notification.isPresent());
		verify(projectionOperations, times(1)).approve(event.payload().requestId(), event.occurredAt());
	}
	...
	private List<InvalidPayloadScenario> invalidPayloads() {
	    return List.of(new InvalidPayloadScenario("blank requestId", "requestId", getBookRequestApprovedEvent("", 1)));
	}

	private IntegrationEventEnvelope<BookRequestApprovedIntegrationPayload> getBookRequestApprovedEvent(String requestId, int schemaVersion) {

		String aggregateId = requestId == null || requestId.isBlank() ? UUID.randomUUID().toString() : requestId;
		
		return new IntegrationEventEnvelope<>(UUID.randomUUID().toString(), IntegrationEventTypes.BOOK_REQUEST_APPROVED,
				"test-handler", aggregateId, AggregateType.BOOK_REQUEST.name(), 0, Instant.now(),
				schemaVersion, new BookRequestApprovedIntegrationPayload(requestId));
	}
}
