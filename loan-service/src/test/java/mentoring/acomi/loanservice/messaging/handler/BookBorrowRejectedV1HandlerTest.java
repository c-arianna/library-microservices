...
public class BookBorrowRejectedV1HandlerTest extends AbstractEventHandlerTest {
    ...
    void shouldRejectPayloadWithBlankIsbn() {
        ...
        IntegrationEventEnvelope<BookBorrowRejectedIntegrationPayload> event = getBookBorrowRejectedEvent("", UUID.randomUUID().toString(), 
        		UUID.randomUUID().toString(), RESERVATION_MISSING, 1);
    ...
    void shouldRejectPayloadWithBlankLoanId() {
        ...
        IntegrationEventEnvelope<BookBorrowRejectedIntegrationPayload> event = getBookBorrowRejectedEvent("9788804336327", "", 
        		UUID.randomUUID().toString(), RESERVATION_MISSING, 1);
	...
	protected IntegrationEventEnvelope<?> differentEvent() {
		 return new IntegrationEventEnvelope<>(UUID.randomUUID().toString(), IntegrationEventTypes.BOOK_RESERVATION_REJECTED, "book-service", 
				 "9788804336327", AggregateType.BOOK.name(), 1, Instant.now(), 1, Map.of());
	}

	@Override
	protected IntegrationEventEnvelope<?> withSchemaVersion(int schemaVersion) {
		return getBookBorrowRejectedEvent("9788804336327", UUID.randomUUID().toString(), UUID.randomUUID().toString(), RESERVATION_MISSING, schemaVersion);
	}
	...
	private IntegrationEventEnvelope<BookBorrowRejectedIntegrationPayload> getBookBorrowRejectedEvent(String isbn, String loanId, 
		...
		return new IntegrationEventEnvelope<>(UUID.randomUUID().toString(), IntegrationEventTypes.BOOK_BORROW_REJECTED, "test-handler", isbn,
