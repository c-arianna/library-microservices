...
public class BookBorrowedV1HandlerTest extends AbstractEventHandlerTest {

	@Mock
    private LoanEventReactor reactor;	
    private BookBorrowedV1Handler handler;

    @BeforeEach
    void setUp() {
        handler = new BookBorrowedV1Handler(reactor, mapper);
    }
    ...
    protected IntegrationEventEnvelope<BookLoanIntegrationPayload> validEvent() {
        return getBookBorrowedEvent("9788804336327", UUID.randomUUID().toString(), UUID.randomUUID().toString(), 1);
    }

    @Override
    protected IntegrationEventEnvelope<?> differentEvent() {
        return new IntegrationEventEnvelope<>(UUID.randomUUID().toString(), IntegrationEventTypes.BOOK_BORROW_REJECTED,
    ...
    protected IntegrationEventEnvelope<?> withSchemaVersion(int schemaVersion) {
        return getBookBorrowedEvent("9788804336327", UUID.randomUUID().toString(), UUID.randomUUID().toString(), schemaVersion);
    }
    
    @Test
    void shouldHandleBookBorrowedEvent() {
        ...
        verify(reactor, times(1)).handleBookBorrowed(command);
    }
    ...
    private List<InvalidPayloadScenario> invalidPayloads() {
	    return List.of(new InvalidPayloadScenario("blank isbn", "isbn", 
	    		             getBookBorrowedEvent("", UUID.randomUUID().toString(), UUID.randomUUID().toString(), 1)),
	    			   new InvalidPayloadScenario("blank loanId", "loanId", getBookBorrowedEvent("9788804336327", "", UUID.randomUUID().toString(), 1)),
	    		       new InvalidPayloadScenario("blank userId", "userId", getBookBorrowedEvent("9788804336327", UUID.randomUUID().toString(), "", 1)));
	}
    	
	private IntegrationEventEnvelope<BookLoanIntegrationPayload> getBookBorrowedEvent(String isbn, String loanId, String userId, int schemaVersion) {

		String aggregateId = isbn == null || isbn.isBlank() ? "9788804336327" : isbn;
		
		return new IntegrationEventEnvelope<>(UUID.randomUUID().toString(), IntegrationEventTypes.BOOK_BORROWED, "test-handler", aggregateId,
