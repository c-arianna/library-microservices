...
public class LoanConfirmedV1Handler extends AbstractEventHandler<LoanIntegrationPayload> {
	...
	public LoanConfirmedV1Handler(@Qualifier("liveLoanProjection") LoanProjectionOperations projectionOperations, 
	...
	protected Optional<ProjectionUpdateNotification> process(LoanIntegrationPayload payload, IntegrationEventEnvelope<?> event) {
		projectionOperations.confirmLoan(payload.loanId(), event.occurredAt());
		
		LocalDate statisticDate = event.occurredAt().atZone(ZoneId.of("Europe/Rome")).toLocalDate();
		dailyLoanStatisticProjectionOperation.registerLoanConfirmed(statisticDate);
