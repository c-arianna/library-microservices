...
public class LoanEventJpaMapper {
	...
	private LoanEvent getEvent(LoanEventType eventType, LoanEventEntity event) {
		...
		case LoanRequested -> {
			...
			yield new LoanRequestedEvent(event.getAggregateId(), event.getEventId(), payload, event.getOccurredAt());
		}
		
		case LoanFailed -> {
			LoanFailedPayload payload = objectMapper.treeToValue(event.getPayload(), LoanFailedPayload.class);
			yield new LoanFailedEvent(event.getAggregateId(), event.getEventId(), payload, event.getOccurredAt());
		}

		case LoanReserved -> {
			LoanPayload payload = objectMapper.treeToValue(event.getPayload(), LoanPayload.class);
			yield new LoanReservedEvent(event.getAggregateId(), event.getEventId(), payload, event.getOccurredAt());
		}
		
		case LoanConfirmed -> {
			LoanPayload payload = objectMapper.treeToValue(event.getPayload(), LoanPayload.class);
			yield new LoanConfirmedEvent(event.getAggregateId(), event.getEventId(), payload, event.getOccurredAt());
		}
		
		case LoanCanceled -> {
			LoanPayload payload = objectMapper.treeToValue(event.getPayload(), LoanPayload.class);
			yield new LoanCanceledEvent(event.getAggregateId(), event.getEventId(), payload, event.getOccurredAt());
		}
		
		case LoanReturned -> {
			LoanPayload payload = objectMapper.treeToValue(event.getPayload(), LoanPayload.class);
			yield new LoanReturnedEvent(event.getAggregateId(), event.getEventId(), payload, event.getOccurredAt());
		}
		
		case LoanConfirmRequested -> {
			LoanPayload payload = objectMapper.treeToValue(event.getPayload(), LoanPayload.class);
			yield new LoanConfirmRequestedEvent(event.getAggregateId(), event.getEventId(), payload, event.getOccurredAt());
		}
	...
	public LoanEventEntity toEntity(LoanEvent event) {
		return new LoanEventEntity(event.aggregateId(), event.eventId(), event.type().name(), toJsonNode(event.payload()), event.occurredAt());
