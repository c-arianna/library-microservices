...
public class UserUpdatedNotificationHandler extends AbstractNotificationHandler {
	...
	public void handleEvent(NotificationEventEnvelope<?> event) {
		...
		UserUpdatedNotificationPayload notificationPayload = new UserUpdatedNotificationPayload(
				requiredField(payload, "userId"), requiredField(payload, "email"), requiredField(payload, "name"),
				requiredField(payload, "lastname"), requiredField(payload, "userIdentityProviderId"), cardNumber, status, role);
